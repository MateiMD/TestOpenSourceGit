 ...
 *     });
 */
Ext.define('Ext.field.SearchSelect', {
	...
	updateValue : function(newValue, oldValue) {
		...
		if(newValue && oldValue && newValue.get(this.getValueField()) != oldValue.get(this.getValueField()) && newValue.get(this.getDisplayField()) === oldValue.get(this.getDisplayField())){
	...
	getTabletPicker : function() {
		...
		var heightList;
		var count=3;
		var clsClass="x-list-label-employee";
		if(this.getStore().getStoreId()=="getOrgChartFieldData") {
				// clsClass='x-list-label-employee';
					if(this.getStore().getData().length>3){
						count=this.getStore().getData().length;
					}
					heightList = (count* 47) + 5;
					if((window.innerHeight - 100) < heightList){
						heightList = (Ext.os.is.BlackBerry && Ext.os.version.getMajor() === 10) ? '12em' : (Ext.os.is.Phone ? '12.5em' : '22em')}
		}else if(this.getStore().getStoreId()=="DataForCopyOrgChart"){
		if(this.getStore().getData().length>3){
						count=this.getStore().getData().length;
					}
			heightList = (count* 47) + 5;
			...
			heightList = (Ext.os.is.BlackBerry && Ext.os.version.getMajor() === 10) ? '12em' : (Ext.os.is.Phone ? '12.5em' : '22em')}
					...
					heightList = (this.getStore().getData().length* 47) +20;
					if((window.innerHeight - 100) < heightList){
						heightList = (Ext.os.is.BlackBerry && Ext.os.version.getMajor() === 10) ? '12em' : (Ext.os.is.Phone ? '12.5em' : '22em')}
		}
		var me=this;
				      var storeRecord = this.getStore();
					  if(!me.duplicateStore){
							me.duplicateStore = Ext.create("Ext.data.Store", {
									model: storeRecord.getModel().getName(), // storeRecord.getModel().displayName // Matei: don't work in production build
									data:storeRecord.getData().items
								});		
							}else{
							me.duplicateStore.removeAll();
							me.duplicateStore.add(storeRecord.getData().items);
							}
		  ...
		  this.listPanel = Ext.create('Ext.Panel', Ext.apply({
			...
			items :[{
				xtype: 'searchfield',
					...
					style : 'font-size: 12px;margin: 4px 4px 4px 4px;border :1px solid #ccc;width:97%;padding-right: 5%;',
					listeners:{
					 keyup:function(field){
					    var value = field.getValue();
						console.log("Filter by " + value );
						me.duplicateStore.clearFilter(!!value);
						if (value) {
						    var thisRegEx = new RegExp(value, "i");
							me.duplicateStore.filterBy(function(record) {
							if(me.duplicateStore.getModel().getFields().items[1].getName()=='text'){
							if(thisRegEx.test(record.get(me.duplicateStore.getModel().getFields().items[1].getName()))) {
									return true;
								};
								return false;
							}else{
							if(thisRegEx.test(record.get(me.duplicateStore.getModel().getFields().items[0].getName()))) {
									return true;
								};
								return false;
							}
					...
					flex:1,
                    itemId: "ddlList",
					store : me.duplicateStore,
					itemTpl : '<span class='+clsClass+'>{' + this.getDisplayField() + ':htmlEncode}</span>',
					listeners : {
		...
		if(me.getUsePicker()) {
			...
			if(value || me.getAutoSelect()) {
				...
				if(record) {
					...
					var el=list.element;
				var cls= list.getSelectedCls();
				var selected = el.down('.' + cls);
				var y = selected.dom.offsetTop;
				list.getScrollable().getScroller().scrollTo(0, (y-10));
				}
	 ...
	 */
	reset : function() {
		var store = this.getStore(), record = (this.originalValue) ? this.originalValue : store.getAt(0);
